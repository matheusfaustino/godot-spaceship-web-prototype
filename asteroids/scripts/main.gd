...
const MAX_OBSTACLES_SPAWN: int = 3
const THRESHOLD_DIFFICULTY_BY_SCORE: int = 50
...
func spawn_asteroid() -> void:
	var can_spawn_another = obstacles.is_empty() or obstacles.back().position.y > randi_range(100, 300)
	if can_spawn_another == false:
		return

	var min_obstacles_spawn = min(score / THRESHOLD_DIFFICULTY_BY_SCORE, MAX_OBSTACLES_SPAWN)
	for _i in randi_range(3, MAX_OBSTACLES_SPAWN):
		...
		var asteroid_x = randi_range(-(asteroid_width / 4), screen_size.x + (asteroid_width / 3))
